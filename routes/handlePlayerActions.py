...
def make_move(data):
    ...
    print(f"Received move request: Player {player_id} wants to move to {move}")
    ...
    # Ensure move is a string - handle any conversion that might be needed
    if isinstance(move, dict) and 'position' in move:
        move = move['position']

    # Verify the move is valid
    valid_move = board._is_valid_move(player_id, move)

    print(f"Move validation result: {valid_move}")

    if not valid_move["result"]:
        error_msg = valid_move.get("message", "Invalid move")
        print(f"Invalid move: {error_msg}")
        emit('error', {'message': error_msg, 'code': 'INVALID_MOVE'})
        return
    ...
    try:
        ...
        if player_obj and player_obj.character:
            character_data = json.loads(player_obj.character)
            old_position = character_data.get('position', None)

        # Perform the move
        lobby.player_move(player_id, move)
        print(f"Move succeeded: Player {player_id} moved from {old_position} to {move}")
...
def handle_suggestion(data):
    ...
    print(f"Received suggestion request: Player {player_id} suggests {suspect} with {weapon}")
    ...
    # Get the board to verify the player is in a room
    board = lobby.get_board()
    if not board:
        emit('error', {'message': 'Game board not found', 'code': 'BOARD_NOT_FOUND'})
        return

    # Verify player is in a room
    location = board._find_player_on_board(player_id)
    if not location or location["type"] != "room":
        emit('error', {'message': 'You must be in a room to make a suggestion', 'code': 'NOT_IN_ROOM'})
        return
    ...
    try:
        ...
        next_to_disprove_name = Player.query.get(next_to_disprove).name if Player.query.get(
            next_to_disprove) else "Unknown"
...
def handle_disprove(data):
    ...
    if next_player_to_try:
        ...
        emit('suggestion_made', {
            ...
            'next_to_disprove_name': next_player_to_try.name,
