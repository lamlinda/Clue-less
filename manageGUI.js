...
document.getElementById('hostLobbyBtn').addEventListener('click', function() {
    ...
    const selectedTheme = themeSelector ? themeSelector.value : "classic";
    ...
    // POST to the /lobby/host endpoint with theme information
    fetch('/lobby/host', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
            name: hostName,
            theme: selectedTheme  // Send the selected theme to server
        })
    })
    ...
    .then(data => {
        ...
        // Show result with theme information
        ...
        hostResultDiv.innerHTML = `<p class="success">Lobby created!</p>` +
                                ...
                                `<p>Theme: <strong>${gameThemes[selectedTheme].name}</strong></p>`;

        console.log("Joining lobby as host with theme", selectedTheme);
        ...
        socket.emit('join_lobby', {
            ...
            theme: selectedTheme  // Pass the theme in the join request
        });
        ...
        // Update character selection UI based on theme
        updateCharacterSelectionUI(selectedTheme);
    })
    .catch(error => {
...
document.getElementById('makeSuggestionBtn').addEventListener('click', function() {
    // Check if player is in a room before allowing suggestion
    if (isInRoom) {
        // Show the suggestion form
        document.getElementById('suggestionForm').style.display = 'block';
        makeSuggestion();
    } else {
        // Display message that you need to be in a room
        const turnResult = document.getElementById('turnResult');
        turnResult.innerHTML = '<p class="error">You must be in a room to make a suggestion</p>';

        // Set a timeout to clear the message after 3 seconds
        setTimeout(() => {
            if (turnResult.innerHTML.includes('must be in a room')) {
...
// When the Make Suggestion button is clicked (from game controls)
document.getElementById('makeSuggestionBtn2').addEventListener('click', function() {
    // Check if player is in a room before allowing suggestion
    if (window.isInRoom) {
        const suggestionForm = document.getElementById('suggestionForm');

        // Toggle the suggestion form visibility
        if (suggestionForm.style.display === 'block') {
            ...
            // Make sure the current room is displayed
            if (window.currentLocation) {
                const currentRoomElement = document.getElementById('currentRoom');
                if (currentRoomElement) {
                    currentRoomElement.textContent = `Room: ${formatPositionName(window.currentLocation)}`;
                }
            }
        }
    } else {
...
// Updated function to update the suggestion button state
function updateSuggestionButton(inRoom, currentLocation) {
    const makeSuggestionBtn = document.getElementById('makeSuggestionBtn2');

    // Store room status in global variables for use in the click handler
    window.isInRoom = inRoom;
    window.currentLocation = currentLocation;

    if (inRoom) {
        // Enable the button when in a room
        makeSuggestionBtn.disabled = false;
        makeSuggestionBtn.classList.remove('disabled');
        makeSuggestionBtn.title = `Make a suggestion about the murder in the ${formatPositionName(currentLocation)}`;

        // Pre-select the room in the suggestion form
        const currentRoomElement = document.getElementById('currentRoom');
        if (currentRoomElement && currentLocation) {
            currentRoomElement.textContent = `Room: ${formatPositionName(currentLocation)}`;
        }
    } else {
        // Disable the button when not in a room
        makeSuggestionBtn.disabled = true;
        makeSuggestionBtn.classList.add('disabled');
        makeSuggestionBtn.title = 'You must be in a room to make a suggestion';
    }
}