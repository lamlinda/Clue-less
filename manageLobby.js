...
socket.on('lobby_created', function(data) {
    ...
    // Store the theme if it's provided
    if (data.theme) {
        console.log("Setting theme from created lobby:", data.theme);
        currentTheme = data.theme;

        // Update the theme selector dropdown to match the lobby's theme
...
socket.on('lobby_joined', function(data) {
    ...
    // IMPORTANT: Set the current theme for ALL joining clients
    if (data.theme) {
        console.log("Setting theme from lobby:", data.theme);
        currentTheme = data.theme;

        // Update the theme selector dropdown to match the lobby's theme (if it exists)
        const themeSelector = document.getElementById('gameTheme');
        if (themeSelector) {
            themeSelector.value = data.theme;
    ...
    lobbyInfoDiv.innerHTML = `<p>Lobby ID: <strong>${data.lobby_id}</strong></p>` +
                       ...
                       `<p>Theme: <strong>${gameThemes[currentTheme].name}</strong></p>`;
    ...
    if (!currentPlayer.select_character) {
        ...
        // Update available characters based on the THEME - this is crucial
        updateCharacterSelectionUI(currentTheme);

        // Then update which ones are already selected
